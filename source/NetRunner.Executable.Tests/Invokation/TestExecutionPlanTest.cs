...
{
    partial class TestExecutionPlanTest
    {
        [GeneratedCode("TestGenerator", "1.0.0.0")]
        private static IEnumerable<TestExecutionPlan> GetInstancesOfCurrentType()
        {
            foreach (var textBeforeFirst in new[]{"", "123"})
            {
                foreach (var parsedTables in ParsedTableTest.CreateNonEmptyObjectsArrays())
                {
                    yield return new TestExecutionPlan(textBeforeFirst, parsedTables);
